...
### 数据库设计
- [ ] 创建营销触发器规则表（marketing_triggers）
- [ ] 创建触发器执行历史表（trigger_executions）
- [ ] 创建用户行为事件表（user_events）

### 触发器类型
- [ ] 用户行为触发器（注册、首单、消费达标、等级升级）
- [ ] 时间触发器（特定时间段、用户生日、节假日）
- [ ] 流失预警触发器（N天未购买、最后一次未完成支付）
- [ ] 地理位置触发器（用户进入门店附近）

### 触发条件配置
- [ ] 用户分群条件（新用户、VIP、普通用户、流失用户）
- [ ] 时间条件（上午/下午/晚上、工作日/周末、特定日期）
- [ ] 地理位置条件（门店半径、城市、区域）
- [ ] 行为条件（消费金额、订单数量、最后购买时间）
